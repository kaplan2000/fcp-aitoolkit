...
<section class="community shell"><div><p class="eyebrow">LET’S BUILD THIS TOGETHER</p><h2>Fresh channels. <span class="serif">More to come.</span></h2><p>We’re just getting started. Follow along for future demos, editing tips and product updates.</p></div><div class="community-links">SOCIAL</div></section>
    ...
    schema = [{'@type': 'SoftwareApplication', '@id': SITE + '/#app', 'name': 'FCP AI Toolkit', 'applicationCategory': 'MultimediaApplication', 'operatingSystem': 'macOS 26.4 or later', 'softwareVersion': '1.0', 'downloadUrl': STORE, 'description': 'A native Final Cut Pro workflow extension for on-device AI captions and Motion title templates. Free template features; automatic AI captions require a subscription.', 'image': SITE + '/images/brand/logo-purple-1024.png', 'datePublished': '2026-06-21', 'publisher': {'@id': SITE + '/#organization'}}]
...
def blog():
    ...
    for post in POSTS:
        ...
        body = f'''<main id="main"><article><header class="article-header shell"><a class="text-link" href="/blog/">← The edit room</a><div class="post-meta"><span class="tag {'tag-lime' if post['version'] == '1.1' else ''}">{post['label']}</span><time datetime="{post['date']}">{human_date(post['date'])}</time><span>FCP AI Toolkit</span></div><h1>{escape(post['title'])}</h1><p class="article-deck">{escape(post['excerpt'])}</p></header><div class="article-layout shell"><aside class="article-aside"><span class="eyebrow">VERSION {post['version']}</span><span class="aside-version">{post['version']}</span><span>{post['label']}</span><a href="{STORE}">View on the App Store ↗</a><a href="{path}index.md">Read as Markdown ↗</a></aside><div class="prose">{post['body']}<div class="article-end"><img src="/images/brand/logo-purple-256.png" width="44" height="44" alt=""><div><strong>FCP AI Toolkit</strong><p>Powered by <a href="https://soleach.com/">Soleach Digital Agency</a></p></div></div></div></div></article><section class="related shell"><p class="eyebrow">KEEP READING</p><a href="/blog/{next_post['slug']}/">{escape(next_post['title'])} ↗</a></section></main>'''
